...
class DiaroScraper:
    ...
    def __init__(self) -> None:
        load_dotenv()
        self.scraper = SeleniumHandler(
            headless=True, raise_exceptions=False, verbose=True, download_driver=True
        )
    ...
    def get_single_diaro_entry(self, driver) -> DiaroEntry:
        actions = [
            DriverAction(
                "get_text", ".InnerContent > div > div:nth-child(1) > div:nth-child(1)", key="date"
            ),
            ...
            DriverAction(
                "get_class",
                ".InnerContent > div > table tr:nth-child(3) .mood_button > i",
                key="mood",
            ),  # dm-1 = awesome
