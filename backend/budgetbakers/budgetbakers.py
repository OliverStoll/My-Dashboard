...
class BudgetBakersDataScraper:
    ...
    def save_payments_to_firestore(self, payment_records: list[PaymentRecord]):
        ...
        for record in payment_records:
            entry_key = f"{record.amount}|{record.category}|{record.recipient}"
            entry_key = entry_key.replace("/", "").replace(".", ",")
            entry_date = record.date.strftime("%Y-%m-%d")
            if entry_date not in payload:
                payload[entry_date] = {}
            payload[entry_date][entry_key] = record.model_dump(mode="json")

        self.firebase.update(ref=f"{self.firebase_path}/records", data=payload)
    ...
    def save_balances_to_firestore(self, balance_records: list[BalanceRecord]):
        ...
        for record in balance_records:
            if record.account not in payload:
                payload[record.account] = {}
            record_date = record.date.strftime("%Y-%m-%d")
            payload[record.account][record_date] = record.model_dump(mode="json")

        self.firebase.update(ref=f"{self.firebase_path}/balances", data=payload)
